...
function generateOrg3() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=org${NEW_ORG}-crypto.yaml --output="../organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    . fabric-ca/registerEnroll.sh $NEW_ORG $PORT
...
function generateOrg3Definition() {
  ...
  export FABRIC_CFG_PATH=$PWD
  set -x
  configtxgen -printOrg Org${NEW_ORG}MSP > ../organizations/peerOrganizations/org${NEW_ORG}.example.com/org${NEW_ORG}.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org${NEW_ORG}.example.com" ]; then
    generateOrg3
  ...
  docker exec cli ./scripts/org3-scripts/updateChannelConfig.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE $NEW_ORG
  if [ $? -ne 0 ]; then
    fatalln "ERROR !!!! Unable to create config tx"
  fi

  infoln "Joining Org3 peers to network"
  docker exec cli ./scripts/org3-scripts/joinChannel.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE $NEW_ORG
  if [ $? -ne 0 ]; then
...
COMPOSE_FILE_COUCH_ORG3=docker/docker-compose-couch-org${NEW_ORG}.yaml
# use this as the default docker-compose yaml definition
COMPOSE_FILE_ORG3=docker/docker-compose-org${NEW_ORG}.yaml
# certificate authorities compose file
COMPOSE_FILE_CA_ORG3=docker/docker-compose-ca-org${NEW_ORG}.yaml
# default image tag
IMAGETAG="latest"
