...
app.post('/upload_to_chain', async function(req, res){
    params = req.body;
    try{
        var result = await contract.submitTransaction("UpdateGrantInfo", params.requester, params.model_id, params.random_number);
        res.send(result);
    }catch(e){
        console.log(e);
        res.send(e);
...
app.post('/model_request', async function(req, res){
    params = req.body;
    var requestEncInfo = await contract.evaluateTransaction("GetRequestEncInfo", params.requester, params.model_id);
    var decrypted1 = key.decrypt(requestEncInfo.code, 'utf8');

    var publicDer = key.exportKey('public');
    var privateDer = key.exportKey('private');

    var encrypt_prik_key = params.random_number;
    var decrypted2 = key.decrypt(encrypt_prik_key, 'utf8');

    if(decrypted1 == decrypted2) res.send("true");
    else res.send("false");
});

app.get('/publicKey', async (ctx, next) => {
    var publicDer = key.exportKey('public');
    var privateDer = key.exportKey('private');
