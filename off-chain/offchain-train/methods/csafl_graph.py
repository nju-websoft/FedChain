...
if __name__ == '__main__':
    ...
    best_acc = 0.0
    save_name = conf["dataset"] + "_result_" + ("iid" if conf["iid"] else ("niid" + str(conf["niid_beta"]))) + "_csafl"
    ...
    for i in range(conf["client_num"]):
        index = np.random.choice(range(conf["cluster_centers"]), 1)[0]
        server.clients_cluster_map[id] = index
        dict[index].append(i)

    server.clusters = []
    for i in range(conf["cluster_centers"]):
        server.clusters.append(np.array(dict[i]))
    ...
    for e in range(1, conf["global_epochs"]):
        ...
        #acc, loss = server.scalesfl_cluster_model_weight_aggregate(candidates, client_datalen, total_len, dataset=dataset)
        ...
        #acc, loss = server.best_cluster_model_weight_aggregate(clients, client_datalen, total_len, dataset=dataset)
        end = time.time()
