...
if __name__ == '__main__':
    ...
    best_acc = 0.0
    save_name = conf["dataset"] + "_result_" + ("iid" if conf["iid"] else ("niid" + str(conf["niid_beta"]))) + "_fedhisyn"
    ...
    for e in range(1, conf["global_epochs"]):
        ...
        for j in range(1):
            for cluster in server.clusters:
                last_local_model = clients[cluster[-1]].local_model
                for i in range(len(cluster)):
                    cur_client = cluster[i]

                    tmp = copy.deepcopy(clients[cur_client].local_model)
                    clients[cur_client].local_train_graph(last_local_model, dataset=dataset)
                    last_local_model = tmp

                    if cur_client in ids:
                        aggregate_ids.append(cluster[(i+1+len(cluster))%len(cluster)])

        for id in aggregate_ids:
            aggregate_models.append(clients[id].local_model)
        ...
        acc, loss = server.model_weight_aggregate([model for model in aggregate_models], client_datalen=client_datalen, total_len=total_len, ids=[c.client_id for c in candidates], dataset=dataset)
