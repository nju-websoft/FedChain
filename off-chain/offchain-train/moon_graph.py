...
if __name__ == '__main__':
    ...
    best_acc = 0.0
    save_name = conf["dataset"] + "_result_" + ("iid" if conf["iid"] else ("niid" + str(conf["niid_beta"]))) + "_moon"
    ...
    for e in range(1, conf["global_epochs"]):
        ...
        #acc, loss = server.model_weight_aggregate([copy.deepcopy(c.local_model) for c in candidates], client_datalen, total_len, [c.client_id for c in candidates], dataset=dataset)
